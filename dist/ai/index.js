 ...
 */
async function analyzeWithGemini$1(component, options = {}) {
  if (!aiProviders.gemini.available) {
 ...
 * @returns {Promise<Object>} Analysis result
 */
async function analyzeWithCohere$1(component, options = {}) {
  if (!aiProviders.cohere.available) {
  ...
  return {
    ...
    semantic_understanding: {
      component_type: classifyComponentType(),
      user_intent: predictUserIntent(),
      content_category: categorizeContent()
    },
    accessibility: {
      level: determineAccessibilityLevel$1(),
      issues: findAccessibilityIssues(component)
    },
    enhancement_suggestions: suggestEnhancements(),
    context_integration: suggestContextIntegration()
  };
}
 ...
 */
function determineAccessibilityLevel$1(component) {
 ...
 */
function determineComplexity$1(component) {
...
function suggestEnhancements(component) {
  // Generic suggestions
  return ['Add semantic metadata', 'Enhance with AI context', 'Improve accessibility'];
}
 ...
 */
async function analyzeComponents(sourceDir, options = {}) {
 ...
 */
async function analyzeComponentWithAI(component, options = {}) {
  if (!isAIAvailable()) {
...
function fallbackAnalysis(component) {
  ...
  const interactions = component.eventHandlers ? component.eventHandlers.filter(handler => handler.startsWith('on')).map(handler => handler.replace(/^on/, '').toLowerCase()) : inferInteractions(component.code || '');
...
async function analyzeWithGemini(component, options) {
  ...
  const semantics = {
    aiGenerated: true,
    aiModel: 'gemini',
    accessibilityLevel: determineAccessibilityLevel(),
    complexity: determineComplexity(component),
...
function extractProps(path) {
  ...
  // Arrow function
  else if (t.isVariableDeclarator(path.node) && t.isArrowFunctionExpression(path.node.init) && path.node.init.params.length > 0) {
...
function inferPurpose(name, code) {
  ...
  if (/button|btn/i.test(nameLower)) {
    return inferButtonPurpose(name);
...
export { aiProviders, analyzeComponentWithAI, analyzeComponents, analyzeWithCohere$1 as analyzeWithCohere, analyzeWithGemini$1 as analyzeWithGemini, getBestProvider, initializeProviders, isAIAvailable };
