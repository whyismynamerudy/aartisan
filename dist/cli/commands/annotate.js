...
function annotateCommand(program) {
  program.command('annotate').description('Enhance components with LLM-powered semantic analysis').argument('[source]', 'Path to source directory to analyze', '.').option('-p, --provider <provider>', 'LLM provider to use (cohere, gemini)', 'cohere').option('-k, --api-key <key>', 'API key for the LLM provider').option('-v, --verbose', 'Enable verbose logging').option('-d, --dry-run', 'Show the proposed changes without applying them').action(async (source, options) => {
