...
import { g as getDefaultExportFromCjs } from '../../index-BrrynTpt.js';
...
import { p as parse_1, i as index, t as traverseModule, _ as _default, g as generateModule, l as libExports } from '../../index-DrWHAuAr.js';
import { validateApiKey, findRelatedFiles, enhanceWithLLM } from '../utils/llm-providers.js';
 ...
 */
function annotateCommand(program) {
  program.command('annotate').description('Enhance components with LLM-powered semantic analysis').argument('[source]', 'Path to source directory to analyze', '.').option('-p, --provider <provider>', 'LLM provider to use (cohere, gemini)', 'cohere').option('-k, --api-key <key>', 'API key for the LLM provider (Cohere)').option('-g, --gemini-api-key <key>', 'API key for the Gemini provider').option('-v, --verbose', 'Enable verbose logging').option('-d, --dry-run', 'Show the proposed changes without applying them').action(async (source, options) => {
    ...
    try {
      ...
      const sourcePath = path$1.resolve(process.cwd(), source);
      ...
      if (options.provider === 'gemini') {
        ...
        if (!options.apiKey) {
          ...
          const envKey = options.provider === 'cohere' ? process.env.COHERE_API_KEY : process.env.GEMINI_API_KEY;
          if (envKey) {
      ...
      const isValidKey = await validateApiKey(options.provider, options.provider === 'gemini' ? options.geminiApiKey || options.apiKey : options.apiKey);
      if (!isValidKey) {
      ...
      for (const component of componentsToAnalyze) {
        ...
        try {
          ...
          const enhancedCode = await enhanceComponentWithLLM(component, context, options);
          if (options.dryRun) {
...
async function findComponentFiles(sourcePath) {
  ...
  const filePatterns = ['**/*.jsx', '**/*.tsx', '**/*.js', '**/*.ts'];
    ...
    try {
      ...
      traverse(ast, {
        ...
        VariableDeclarator(path) {
          if (libExports.isArrowFunctionExpression(path.node.init)) {
            const directive = hasAartisanDirective(path);
...
function hasAartisanDirective(path) {
  ...
  if (libExports.isVariableDeclarator(node) && path.parentPath.node.leadingComments) {
...
async function gatherCodebaseContext(components, allFiles, options) {
  ...
  for (const component of components) {
    ...
    const fileContents = await Promise.all(allFiles.filter(fp => fp !== component.filePath).map(async filePath => {
      try {
...
async function enhanceComponentWithLLM(component, context, options) {
  ...
  const distRootDir = path$1.resolve(__dirname, '../../..');
  console.log(chalk.blue(`[DEBUG] Dist root directory: ${distRootDir}`));
  const reactHooksFile = path$1.join(distRootDir, 'dist/react/hooks.js');
  const reactDirectivesFile = path$1.join(distRootDir, 'dist/react/directives.js');
  const hocFile = path$1.join(distRootDir, 'dist/react/hoc.js');
  const defineComponentFile = path$1.join(distRootDir, 'dist/react/defineComponent.js');
  ...
  const contextSummary = context.map(file => `// Context from ${path$1.basename(file.filePath)}:\n${file.content.slice(0, 500)}...`).join('\n\n');
  ...
  const prompt = createEnhancementPrompt(component, contextSummary);
...
async function applyEnhancement(filePath, enhancedCode) {
  ...
  const ast = parse_1(originalCode, {
    sourceType: 'module',
    plugins: ['jsx', 'typescript']
  });
  ...
  traverse(ast, {
    ...
    VariableDeclaration(path) {
      for (const declarator of path.node.declarations) {
        if (libExports.isVariableDeclarator(declarator) && libExports.isArrowFunctionExpression(declarator.init) && hasAartisanDirective(path)) {
          componentStart = path.node.start;
          componentEnd = path.node.end;
          componentFound = true;
  ...
  if (enhancedImports.length > 0) {
    ...
    newCode = combinedImports + originalCode.substring(importEndPosition, componentStart) + componentContent + originalCode.substring(componentEnd);
  } else {
    // Just replace the component section
    newCode = originalCode.substring(0, componentStart) + enhancedCode + originalCode.substring(componentEnd);
  }
...
function extractComponentContent(code) {
  ...
  traverse(ast, {
    ...
    VariableDeclaration(path) {
      if (componentStart === 0) {
        for (const declarator of path.node.declarations) {
          if (libExports.isVariableDeclarator(declarator) && (libExports.isArrowFunctionExpression(declarator.init) || libExports.isFunctionExpression(declarator.init))) {
            componentStart = path.node.start;
            path.stop();
          }
        }
