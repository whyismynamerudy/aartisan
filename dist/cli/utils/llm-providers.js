import { g as getDefaultExportFromCjs, s as srcExports } from '../../index-BrrynTpt.js';
...
async function cohereRerank(query, documents, options) {
  ...
  try {
    ...
    console.log(chalk.blue('[DEBUG] API request payload size: ~' + Math.round(JSON.stringify({
      query,
  ...
  try {
    ...
    const response = await axios({
      ...
      data: {
        ...
        // Using the latest model from the docs
        messages: [{
          role: 'user',
          content: prompt
        }]
    ...
    if (Array.isArray(messageContent)) {
      ...
      generatedText = response.data.message?.text || response.data.message || response.data.response || response.data.content || JSON.stringify(response.data);
    }
...
export { cohereGenerate, cohereRerank, enhanceWithLLM, findRelatedFiles, geminiGenerate, validateApiKey };
