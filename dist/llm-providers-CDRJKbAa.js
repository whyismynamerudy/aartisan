import { g as getDefaultExportFromCjs, s as srcExports } from './index-bpYt4yzm.js';
import require$$1 from 'util';
import Stream$3, { Readable } from 'stream';
 ...
 * @returns {Promise<Object>} Cohere client instance
 */
async function initCohereClient(apiKey) {
  try {
    if (!CohereClient) {
      console.log(chalk.blue('[DEBUG] Dynamically importing cohere-ai library'));
      // Use dynamic import to avoid ESM issues
      const {
        CohereClientV2
      } = await import('./index-CzuDoCE3.js').then(function (n) { return n.i; });
      CohereClient = CohereClientV2;
      console.log(chalk.blue('[DEBUG] Successfully imported cohere-ai library'));
    }
    return new CohereClient({
      token: apiKey
    });
  } catch (error) {
    console.error(chalk.red(`[ERROR] Failed to initialize Cohere client: ${error.message}`));
    if (error.code === 'ERR_MODULE_NOT_FOUND') {
      console.error(chalk.red('[ERROR] cohere-ai module not found. Please install it using: npm install cohere-ai'));
    }
    throw error;
  }
}
...
async function cohereRerank(query, documents, options) {
  ...
  // Fallback to axios implementation if documents are empty
  if (!documents || documents.length === 0) {
    console.log(chalk.yellow('[DEBUG] No documents to rerank, returning empty array'));
    return [];
  }
  try {
    // If too many documents, use axios direct approach - it's more efficient for bulk reranking
    if (documents.length > 20) {
      return await cohereRerankWithAxios(query, documents, options);
    }

    // Initialize Cohere client
    console.log(chalk.blue('[DEBUG] Initializing Cohere client'));
    const cohere = await initCohereClient(options.apiKey);
    ...
    // Call Cohere's rerank API using the official client
    console.log(chalk.blue('[DEBUG] Calling Cohere rerank API with model: rerank-english-v3.0'));
    console.log(chalk.blue('[DEBUG] API request documents count: ' + documentTexts.length));
    const startTime = Date.now();

    // Convert documents to the format expected by the Cohere client
    const cohereDocuments = documentTexts.map(text => ({
      text
    }));
    const response = await cohere.rerank({
      ...
      topN: 5 // Get top 5 documents
    });
    const duration = Date.now() - startTime;
    console.log(chalk.green(`[DEBUG] Cohere rerank API response received in ${duration}ms`));
    console.log(chalk.green(`[DEBUG] Rerank results count: ${response.results.length}`));
    ...
    // Try fallback to axios if client fails
    try {
      console.log(chalk.yellow('[DEBUG] Trying fallback to direct axios implementation'));
      return await cohereRerankWithAxios(query, documents, options);
    } catch (fallbackError) {
      console.error(chalk.red(`[ERROR] Fallback rerank also failed: ${fallbackError.message}`));
      console.log(chalk.yellow('[DEBUG] Falling back to basic filtering'));

      // Final fallback to basic filtering 
      return documents.filter(doc => doc.content.toLowerCase().includes(query.toLowerCase())).slice(0, 3).map(doc => ({
...
/**
 * Rerank documents using direct axios calls to Cohere API
 * @param {string} query - Query string
 * @param {Array<{filePath: string, content: string}>} documents - Array of documents
 * @param {Object} options - Options including API key
 * @returns {Promise<Array>} Reranked documents 
 */
async function cohereRerankWithAxios(query, documents, options) {
  console.log(chalk.blue('[DEBUG] Using direct axios implementation for rerank'));
  try {
    // Prepare documents for the rerank API
    const documentTexts = documents.map(doc => doc.content);

    // Call Cohere's rerank API directly with axios
    const startTime = Date.now();
    const response = await axios.post('https://api.cohere.com/v2/rerank', {
      model: 'rerank-english-v3.0',
      query,
    ...
    const duration = Date.now() - startTime;
    console.log(chalk.green(`[DEBUG] Axios rerank API response received in ${duration}ms`));
    ...
    console.error(chalk.red(`[ERROR] Axios rerank API error: ${error.message}`));
    if (error.response) {
 ...
 */
async function cohereGenerate(prompt, options) {
  ...
  try {
    ...
    try {
      ...
      return extractCodeFromText(generatedText);
    } catch (clientError) {
      // If client fails, fall back to axios
      console.error(chalk.red(`[ERROR] Cohere client error: ${clientError.message}`));
      console.log(chalk.yellow('[DEBUG] Falling back to axios implementation'));
      return await cohereGenerateWithAxios(prompt, options);
    }
  } catch (error) {
    console.error(chalk.red(`[ERROR] All Cohere generate methods failed: ${error.message}`));
    throw new Error(`Failed to generate with Cohere: ${error.message}`);
  }
}

/**
 * Generate content using direct axios calls to Cohere API
 * @param {string} prompt - LLM prompt
 * @param {Object} options - Options including API key
 * @returns {Promise<string>} Generated code
 */
async function cohereGenerateWithAxios(prompt, options) {
  console.log(chalk.blue('[DEBUG] Using direct axios implementation for chat'));
  try {
    const startTime = Date.now();
    ...
    const duration = Date.now() - startTime;
    console.log(chalk.green(`[DEBUG] Axios chat API response received in ${duration}ms`));
    ...
    console.error(chalk.red(`[ERROR] Axios chat API error: ${error.message}`));
    if (error.response) {
      console.error(chalk.red(`[ERROR] Status: ${error.response.status}`));
      console.error(chalk.red(`[ERROR] Response data: ${JSON.stringify(error.response.data)}`));
    }
 ...
 * @param {string} generatedText - Generated text from LLM
 * @returns {string} Extracted code
 */
function extractCodeFromText(generatedText) {
...
async function validateApiKey(provider, apiKey) {
  ...
  try {
    if (provider === 'cohere') {
      // Use the simpler axios approach for key validation
      console.log(chalk.blue('[DEBUG] Testing Cohere API key with models endpoint'));
      const startTime = Date.now();
      const response = await axios.get('https://api.cohere.com/v2/models', {
        headers: {
 ...
 */
async function findRelatedFiles(query, documents, options) {
  ...
  if (options.provider === 'cohere') {
    ...
    const filteredDocs = documents.filter(doc => doc.content.toLowerCase().includes(query.toLowerCase())).slice(0, 5).map(doc => ({
      filePath: doc.filePath,
      content: doc.content,
      relevance: 0.5 // Placeholder score
    }));
 ...
 */
async function enhanceWithLLM(prompt, options) {
...
export { requireCallBindApplyHelpers as a, form_data as b, cohereRerank as c, cohereGenerate as d, enhanceWithLLM as e, findRelatedFiles as f, getIntrinsic as g, geminiGenerate as h, requireType as r, validateApiKey as v };
//# sourceMappingURL=llm-providers-CDRJKbAa.js.map
