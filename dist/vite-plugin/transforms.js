 ...
 */
async function transformJSX(code, fileName, options) {
  // Parse the code
      ...
      path.traverse({
        ...
        JSXElement(jsxPath) {
          ...
          if (comments.some(comment => comment.includes('@aartisan'))) {
            processCommentDirectives(jsxPath, comments, metadata);
          }
        }
      });
    }
...
function processComponent(path, metadata, options) {
  ...
  } else if (t.isVariableDeclarator(path.node) && path.node.init.params && path.node.init.params.length > 0) {
...
function processCommentDirectives(path, comments, metadata, options) {
  ...
  if (Object.keys(directives).length > 0) {
    ...
    openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier('data-aartisan'), t.stringLiteral('true')));

    // Add other directive attributes
    Object.entries(directives).forEach(([key, value]) => {
      if (value !== true) {
        openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier(`data-aartisan-${key}`), t.stringLiteral(value)));
      }
    });
...
function addDataAttributes(path, componentName, purpose) {
  path.traverse({
    JSXElement(jsxPath) {
      ...
      if (jsxPath.parent && (t.isReturnStatement(jsxPath.parent) || t.isArrowFunctionExpression(jsxPath.parent))) {
        ...
        openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier('data-aartisan-component'), t.stringLiteral(componentName)));

        // Add purpose attribute
        if (purpose) {
          openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier('data-aartisan-purpose'), t.stringLiteral(purpose)));
        }
      }
    }
  });
 ...
 */
async function generateMetadata(metadataCache, options) {
...
export { generateMetadata, transformJSX };
